...
public class Snake {
   ...
   public void draw(Game game){
      Color alive = isAlive ? Color.BLACK : Color.RED;
      for (int i = 0; i< snakeParts.size(); i++ )
      {   GameObject part = snakeParts.get(i);
         String sign = (i!=0) ? BODY_SIGN : HEAD_SIGN;
         game.setCellValueEx(part.x, part.y, Color.NONE, sign, alive, 75);
      }
   }
...
// also moves all elements
   ...
   public boolean checkCollision(GameObject headCol)
   {
      for (GameObject snakePart : snakeParts) {
         if (headCol.x == snakePart.x
                 && headCol.y == snakePart.y) {
            return true;
         }
      }
      return false;
