...
public class Rocket extends GameObject {
   ...
   {   super.draw(game);   // Call method of super class
      downFire.draw(game);
      rightFire.draw(game);
      leftFire.draw(game);
   }
   ...
   public Rocket(double x, double y) {
      super(x, y, ShapeMatrix.ROCKET);    // Arrays.asList
      this.downFire  = new RocketFire(Arrays.asList(ShapeMatrix.FIRE_DOWN_1, ShapeMatrix.FIRE_DOWN_2, ShapeMatrix.FIRE_DOWN_3));
      this.leftFire  = new RocketFire(Arrays.asList(ShapeMatrix.FIRE_SIDE_1, ShapeMatrix.FIRE_SIDE_2));
      this.rightFire = new RocketFire(Arrays.asList(ShapeMatrix.FIRE_SIDE_1, ShapeMatrix.FIRE_SIDE_2));
   }
   ...
   {   downFire.x = x + (width / 2);
      downFire.y = y + height;
      downFire.show(); // Why without class name?
   } else
   {   downFire.hide();
   }
